...
class _SearchResultPageState extends State<SearchResultPage> {
  ...
  final DatabaseReference _profileRef = FirebaseDatabase.instance.ref().child('profile'); // Reference to profile node
  ...
  Future<String> _getCreatorName(String idCreator) async {
    ...
    if (snapshot.value != null) {
      Map<dynamic, dynamic>? profiles = snapshot.value as Map<dynamic, dynamic>?;
      if (profiles != null) {
        for (var profile in profiles.entries) {
          if (profile.value is Map<dynamic, dynamic> && profile.value['username'] == idCreator) {
            return profile.value['nama'] ?? ''; // Adjust 'name' based on your database structure
          }
        }
      }
    }
              ...
              ? ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return FutureBuilder(
                      future: _getCreatorName(resep['id_creator'] ?? ''), // Pass id_creator here
                      builder: (BuildContext context, AsyncSnapshot<String> snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          String creatorName = snapshot.data ?? ''; // Retrieved creator's name
  ...
  Widget _buildRecipeCard(Map<dynamic, dynamic> resep, String creatorName) {
    String imageName = resep['foto'] ?? ''; // Replace with actual field name in Firebase
    String deskripsi = resep['deskripsi'] ?? '';

    // Constructing image path relative to pubspec.yaml
    String imageUrl = 'lib/images/$imageName'; // Adjust path as per your folder structure
